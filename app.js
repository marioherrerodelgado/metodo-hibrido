...
window.doLogin=async function(){
  ...
  if(!e||!p){err.textContent="Introduce email y contrasena.";err.classList.add("show");return;}
  btn.disabled=true;ld.classList.add("show");
  try{await signInWithEmailAndPassword(auth,e,p);}catch(x){
    let m="Error al iniciar sesion.";
    if(x.code==="auth/user-not-found"||x.code==="auth/invalid-credential")m="Email o contrasena incorrectos.";
    else if(x.code==="auth/too-many-requests")m="Demasiados intentos. Espera unos minutos.";
    else if(x.code==="auth/invalid-email")m="Email no valido.";
...
function iA(){const now=new Date(),h=now.getHours();document.getElementById("gt").textContent=h<13?"Buenos dias â˜€ï¸":h<20?"Buenas tardes ðŸ’ª":"Buenas noches ðŸŒ™";goToday();}
...
function rM(){
  ...
  for(let d=1;d<=dim;d++){const wl=wm[d]||[];if(!wl.length&&fl!=="all")continue;const iT=isCur&&d===td.getDate(),dt=new Date(Y,M,d);
    h+='<div class="dl"'+(iT?' id="ts"':'')+'>'+DF[dt.getDay()]+", "+d+" "+MN[M]+(iT?" â€” Hoy":"")+'</div>';
    ...
    else{wl.forEach(w=>{const sc=gSC(w),isC=(w.type||"").toLowerCase().includes("compet"),iCl="i"+({baja:"b",media:"m",alta:"a",maxima:"x"}[w.intensity||"media"]),iLb={baja:"Suave",media:"Moderado",alta:"Exigente",maxima:"Maxima"}[w.intensity||"media"]||"Moderado",tt=w.titulo||"Entrenamiento",du=w.duration||"",ty=w.type||"",pr=(w.main||"").split("\n").filter(l=>l.trim()).slice(0,3),wd=JSON.stringify(w).replace(/'/g,"&#39;");
      h+='<div class="wc" onclick=\'oD('+wd+')\'><div class="wcb '+sc+'"></div><div class="wci"><div class="wct"><div class="wcs"><div class="wcd '+sc+'"></div><span class="wsn '+sc+'">'+(isC?"Competicion":gSL(w))+'</span></div><span class="wi '+iCl+'">'+iLb+'</span></div><div class="wtt">'+tt+'</div><div class="wch">'+(du?'<div class="ch">'+du+'</div>':'')+(ty&&!isC?'<div class="ch">'+ty+'</div>':'')+'</div>'+(pr.length?'<div class="wp">'+pr.map(l=>'<div class="wpi"><div class="wpd"></div>'+l.trim()+'</div>').join("")+'</div>':'')+'<button class="wb">Ver entrenamiento completo</button></div></div>';});}}
  if(!h)h='<div style="text-align:center;padding:48px 20px;color:var(--t3);font-size:14px">Sin entrenamientos este mes</div>';
  document.getElementById("msc").innerHTML='<div class="ca">'+h+'</div>';if(isCur)setTimeout(gT,80);
}
...
function gT(){const e=document.getElementById("ts");if(e)e.scrollIntoView({behavior:"smooth",block:"start"});const t=document.querySelector(".wd.tod");if(t){t.classList.add("sel");t.scrollIntoView({inline:"center",block:"nearest",behavior:"smooth"});}}
...
window.oD=function(w){const ds=gD(w),pts=ds.split("-"),d=parseInt(pts[2]),mo=parseInt(pts[1])-1,dt=new Date(parseInt(pts[0]),mo,d),sc=gSC(w),isC=(w.type||"").toLowerCase().includes("compet");
  document.getElementById("dpi").textContent=isC?"Competicion":gSL(w);document.getElementById("dpi").className="dp "+sc;
  ...
  const iLb={baja:"Suave",media:"Moderado",alta:"Exigente",maxima:"Maxima"}[w.intensity||"media"]||"Moderado",iCl={baja:"var(--gr)",media:"var(--bl)",alta:"var(--or)",maxima:"var(--or)"}[w.intensity||"media"]||"var(--bl)";
  document.getElementById("dst").innerHTML='<div class="dsi"><div class="dsl">Duracion</div><div class="dsv">'+(w.duration||"â€”")+'</div></div><div class="dsi"><div class="dsl">Volumen</div><div class="dsv" style="font-size:13px">'+(w.volume||"â€”")+'</div></div><div class="dsi"><div class="dsl">Nivel</div><div class="dsv" style="font-size:13px;color:'+iCl+'">'+iLb+'</div></div>';
...
function bCP(){const now=new Date();now.setHours(0,0,0,0);const filtered=cf==="all"?COMPS:COMPS.filter(c=>c.cat===cf);let up="",pa="";
  filtered.forEach(c=>{const dt=new Date(c.date+"T00:00:00"),d=parseInt(c.date.split("-")[2]),mo=parseInt(c.date.split("-")[1])-1,ip=dt<now,cat=c.cat;
    const card='<div class="cc'+(ip?" past":"")+'"><div class="ccb '+cat+'"></div><div class="cci"><div class="cd"><span class="cdd">'+d+'</span><span class="cdm">'+MS[mo]+'</span></div><div class="cn"><div class="cnn">'+c.name+'</div><div class="cnm">'+c.lugar+(c.note?" â€” "+c.note:"")+'</div></div><span class="cdd2 '+cat+'">'+c.dist+'</span></div></div>';
    if(ip)pa+=card;else up+=card;});
  document.getElementById("cbd").innerHTML=(up?'<div class="sh">Proximas</div>'+up:'')+(pa?'<div class="sh">Pasadas</div>'+pa:'')+(!up&&!pa?'<div style="text-align:center;padding:40px;color:var(--t3)">Sin competiciones</div>':'');}
